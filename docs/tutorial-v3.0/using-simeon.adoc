...
This tutorial works in version 3.0 of Simeon which can be found under
...
https://github.com/The-Blockchain-Company/simeon/blob/master/semantics-3.0/src/Language/Simeon/Examples/EscrowSimpleV2.hs[`+EscrowSimmpleV2.hs+`].
...
This shows that we're now in a contract where the choice is up to Carol, and that there is still the 450 Entropic in the `"alice"` account.
...
We look next at our tool, the Simeon Playground, that
...
==== link:./embedded-simeon.adoc[Prev] link:./README.adoc[Up] link:./playground-overview.adoc[Next]
