...
be run. The code that we describe here comes from the Haskell modules https://github.com/The-Blockchain-Company/simeon/blob/master/semantics-3.0/src/Language/Simeon/Semantics.hs[Semantics.hs] and https://github.com/The-Blockchain-Company/simeon/blob/master/semantics-3.0/src/Language/Simeon/Util.hs[Util.hs].

=== Simeon

The Simeon domain-specific language (DSL) is modelled as a collection of  algebraic
...
We saw link:./simeon-step-by-step.adoc[in the previous tutorial] what these contracts do. In the rest of this tutorial we will dig a bit deeper into the Haskell types that are used to represent the various components of the contracts, including accounts, values, observations, and actions. We will also look at types that relate to the execution of contracts, including inputs, states, the environment. 

=== Basic components

In modelling basic parts of Simeon we use a combination of Haskell `data` types, that define _new_ types, and `type` synonyms that give a new name to an existing type.footnote:[In fact we used `newtype` declarations rather than `data` types because they are more efficiently implemented.] So, members of `PubKey` and `Party` will look like `(PubKey "alice")`, `(PubKey "bob")` and so on. 
[source,haskell]
----
...
This completes our discussion of the types that make up Simeon contracts.

=== Dynamic data

As we noted earlier, the semantics of Simeon consist in building _transactions_, like this:
[#img-transaction]
[caption="Figure 1: "]
...
This is part of the architecture of Bcc/Zerepoch, which acknowledges that it is not possible to predict precisely in which slot a particular transaction will be processed. Transactions are therefore given a _slot interval_ in which they are expected to be processed, and this carries over to Simeon: each step of a Simeon contract is processed in the context of a range of slots.
[source,haskell]
----
data Slot         = Slot Integer 
...
----
How does this affect the processing of a Simeon contract? Each step is processed relative to a slot interval, and the current slot value needs to lie within that interval. 
...
by a layer external to Simeon, and so they need not be modelled
explicitly.
...
Simeon strict in all arguments to all constructors, so that Simeon
contracts are wholly _finite_ data structures, with no partial or
infinite components.

==== link:./simeon-step-by-step.adoc[Prev] link:./README.adoc[Up] link:./embedded-simeon.adoc[Next]
