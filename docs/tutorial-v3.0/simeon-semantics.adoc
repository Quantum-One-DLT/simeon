...
This tutorial gives a high-level formal semantics of Simeon 2.0 by
describing the types of inputs supported, and the main semantic
functions.

In general, participants in a Simeon contract communicate with the
...
As we mentioned in link:./simeon-data.adoc[the previous tutorial], inputs
...
The effect of executing a transaction in a Simeon contract is modelled
...
like DoS. For these reasons, the Simeon 2.0 semantics disallows
providing information that is not required.

Other than that, the only thing that Simeon does when provided with
`+Choice+`s and `+Oracle+`s is to record them in the state so that the
`+reduce+` function can access them.

=== Combinators and `+Null+`

In this section, we describe the semantics of the remaining Simeon
...
It is worth noting that, because Simeon follows a `pull' model, it is
...
However, we have written Simeon semantics so that the most recent
...
==== link:./simeon-data.adoc[Prev] link:./README.adoc[Up] link:./embedded-simeon.adoc[Next]
