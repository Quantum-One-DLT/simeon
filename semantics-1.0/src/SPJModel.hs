...
data Currency = GBP | USD | BCC deriving (Eq, Show)
...
translateSPJContractToSimeon :: M.Person -> M.Person -> Contract -> M.Contract
translateSPJContractToSimeon me counterparty c = case c of
    Zero -> M.Null
    One -> M.Pay (M.IdentPay 1) counterparty me (M.Value 1) maxTimeout M.Null -- TODO should we commit first?
    Give contract -> translateSPJContractToSimeon counterparty me contract   -- swap me/counterparty
  ...
  where
    go = translateSPJContractToSimeon me counterparty
...
-- Simeon implementation of zero-coupon bond example from SPJ paper
zcbSimeon block cash me person = do
...
rainInCyprusSimeonContract block me person = do
...
europeanSimeon :: M.Timeout -> M.Contract -> M.Contract
europeanSimeon t c = M.When M.FalseObs t M.Null c

american :: Integer -> Integer -> Contract -> Contract
american t1 t2 u = Anytime (between t1 t2) u


americanSimeon :: M.Timeout -> M.Timeout -> M.Contract -> M.Contract
americanSimeon t1 t2 u = M.When M.FalseObs t1 M.Null (M.When M.TrueObs t2 u M.Null)
...
limitContractSimeon :: M.Timeout -> M.Timeout -> M.Contract -> M.Contract
limitContractSimeon t1 t2 c = do
    let interestRateObs = M.ValueGE (M.ValueFromOracle "interestRate" (M.Value 0)) (M.Value 6)
        obs = M.NotObs interestRateObs
    M.While obs maxTimeout (americanSimeon t1 t2 c) M.Null
