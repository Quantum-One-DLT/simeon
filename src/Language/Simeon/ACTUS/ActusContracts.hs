...
module Language.Simeon.ACTUS.ActusContracts where
...
import Language.Simeon.ACTUS
import Language.Simeon

dayToSlot :: Day -> Slot
dayToSlot d = let
    (MkSystemTime secs _) = utcToSystemTime (UTCTime d 0)
    in Slot (fromIntegral secs - bccEpochStart `mod` 20)
    ...
    generator (f, state) (day, events) =
        let (daysum, newState) = foldl
            ...
            cont = if amount == 0 then id
                   else \contract -> f $ When [Case (Deposit acc from bcc $ Constant amount)
                        (Pay acc (Party to) bcc (Constant amount) contract)] (dayToSlot day) Close
            in (cont, newState)
...
genPrincialAtMaturnityGuaranteedContract investor issuer guarantor config@ContractConfig{..} = guaranteed
  where
    ...
    guaranteed = When [Case (Deposit acc guarantor bcc $ Constant totalPayoff) cont]
                startDate Close
    ...
    generator (f, state) (day, events) =
        let (daysum, newState) = foldl
            ...
            cont = if amount == 0 then id
                        ...
                        (Pay acc (Party to) bcc (Constant amount)
                            (if from == investor then contract
                            else Pay acc (Party guarantor) bcc (Constant amount) contract))]
                        (dayToSlot day) Close
            in (cont, newState)
