...
module Language.Simeon.Examples.CrowdFunding where
...
crowdfunding target maxContrib tim p lp =
  ...
  where cont = If (ValueGE (addAll srcAvail) (Constant target))
                  ...
                  Close
        srcAccs = lp
        srcAvail = map (`AvailableMoney` bcc) srcAccs
        creatorAcc = p
...
payAll acId payee cont =
  If (ValueGT (AvailableMoney acId bcc) (Constant 0))
     (Pay acId payee bcc (AvailableMoney acId bcc) cont)
     cont
...
transitionFunction _ (party, True) =
  (Nothing,
   Deposit party party bcc (ChoiceValue (ChoiceId "1" party)))
