module Language.Simeon.SemanticsSerialisation (contractToByteString) where

import           Data.Ratio (denominator, numerator)
import qualified Data.Text.Encoding as Text
import           Language.Simeon.ExtendedBuilder (ExtendedBuilder)
import qualified Language.Simeon.ExtendedBuilder as ExtendedBuilder
import           Language.Simeon.SemanticsTypes (Action (..), Bound (Bound), Case (..), ChoiceId (..), Contract (..), Observation (..), Party (..), Payee (..), Slot (Slot), Token (..), Value (..), ValueId (..))
import           Language.Simeon.Serialisation (intToByteString, listToByteString, packByteString, positiveIntToByteString)
