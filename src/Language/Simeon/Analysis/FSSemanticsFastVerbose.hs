module Language.Simeon.Analysis.FSSemanticsFastVerbose where
...
import           Language.Simeon.SemanticsTypes
...
-- foldWithKey for Zerepoch' AssocMap
foldAssocMapWithKey :: (a -> k -> b -> a) -> a -> Map k b -> a
foldAssocMapWithKey f acc = foldl' decF acc . M.toList
  where decF a (k, v) = f a k v

-- Convert Zerepoch' AssocMap into a Map with symbolic values, which are literals of
...
-- It generates variables, runs SBV, and it interprets the result in Simeon terms.
