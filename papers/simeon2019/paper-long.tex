...
\begin{document}
%
\title{Simeon: implementing and analysing financial contracts on blockchain}%\thanks{Supported by BCCCOIN.}}
%
...
%
\institute{BCCCOIN, Hong Kong, 
\email{alexander.nemish@tbco.io, pablo.lamela@tbco.io, simon.thompson@tbco.io,~david.smith@tweag.io},  \and
...
\vspace{-4mm}
\begin{abstract}
Simeon is a DSL for financial contracts. We describe the implementation of Simeon on the Bcc blockchain, and the Simeon Playground web-based development and simulation environment.

Contracts in Simeon can be exhaustively analysed prior to running them, thus providing strong guarantees to participants in the contract. The Simeon system itself has been formally verified using the Isabelle theorem prover, establishing such properties as the conservation of money.

\keywords{Bcc \and DSL \and functional \and Haskell \and SMT \and static analysis}
\end{abstract}


\section{Introduction}

Simeon\footnote{The complete Simeon project, including semantics and implementation, is available at \url{https://github.com/The-Blockchain-Company/simeon}.} is a domain-specific language (DSL) for implementing financial contracts on blockchain: our initial target is Bcc, but it could be implemented on many distributed ledgers (DLT platforms), including Ethereum. Simeon is embedded in Haskell, allowing users selectively to use aspects of Haskell -- typically definitions of constants and simple functions -- to express contracts more readably and succinctly. Section \ref{sec:overview} gives an overview of the language, and the changes made to it since it was originally introduced in ~\cite{isola-simeon}.

Simeon is specified by a reference semantics for the language written in Haskell, and we can use that in a number of ways. We can interpret Simeon contracts in Haskell itself, but we can also use that implementation, compiled into Zerepoch~\cite{ZerepochPlatform}, to interpret Simeon directly on the Bcc blockchain, see Section \ref{sec:implementation}. We can also execute the semantics -- translated into PureScript -- directly in a browser, to give an interactive simulation environment, see Section~\ref{sec:playground}.

Because Simeon is a DSL, we are able to build special purpose tools and techniques to support it. Crucially in a financial environment, we are able to \emph{exhaustively analyse contracts} without executing them, so that we can, for instance, check whether any particular contract is able to make all the payments it should: in the case it is not, we get an explicit example of how it can fail. This analysis, reported in Section \ref{sec:static}, is built into the Simeon Playground. Finally, we are able to use formal verification to prove properties of the implementation of Simeon, including a guarantee that ``money in $=$ money out'' for all contracts; see Section~\ref{sec:verification}.

\section{Simeon overview}
\label{sec:overview}
...
\subsubsection*{The Simeon model}
...
It is through their wallets that users are able to interact with Simeon contracts running on the blockchain, making deposits and receiving payments. %Note, however, that these are definitely off chain actions that need to be controlled by code running in the user's wallet: they cannot be made to happen by the Simeon contract itself.

%\subsection{Executing a Simeon contract}
%At each slot, a running Simeon contract will receive a list of inputs in order. The contract is executed by evaluating it step by step until it cannot be changed any further without processing any input, a condition that we call being quiescent. The first input is then processed, and then the contract is single stepped again until quiescence, and this process is repeated until all the inputs are processed. At each step the current contact and the state will change, some input may be processed, and payments made.


\subsubsection*{Simeon step by step}

Simeon has five ways of building contracts, we call these \emph{contract constructs}. \emph{Contract constructs}, in turn, can also contain \emph{values}, \emph{observations} and \emph{actions}.
...
\texttt{Value}s include some quantities that change with time, including the current slot number, the current balance of an account (in Entropic), and any choices that have already been made. % ; we call these volatile values. - PLS: this definition is no longer used
...
\section{Implementation of Simeon on Bcc}
\label{sec:implementation}

Simeon is specified by an executable semantics written in Haskell, but to make it usable in practice to execute financial contracts, it needs to be implemented on a blockchain. In this section, we explain how Simeon is executed on the Bcc blockchain using an interpreter written in the Zerepoch programming language.

\subsection{Bcc and Zerepoch}

Bcc is a third-generation blockchain that solves the energy usage
issue by moving to an energy efficient \emph{Proof of Stake} protocol~\cite{Shardagnostic-Genesis}.

Bcc aims to support smart contracs during its Sophie release in 2020.
Bcc smart contract platform is called \emph{Zerepoch}, and
it uses Haskell programming language to generate a form of $System F_{\omega}$,
called \emph{Zerepoch Core}, by extending GHC using its plugin
support~\cite[Section 13.3]{ghcusersguide}.

To implement Simeon contracts, we use the ZerepochTx compiler,
which compiles Haskell code into serialized \emph{Zerepoch Core} code,
to create a Bcc \emph{validator script} that ensures the correct execution of the contract.
This form of implementation relies on the extensions to the UTxO model described in~\cite{ZerepochPlatform}.


\subsection{Extended UTxO}

Bcc is a UTxO-based (unspent transaction output) blockchain, similar to Bitcoin~\cite{sok}.
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=4in]{figures/Simeon3-Figures-003.jpeg}
    \caption{Bcc EUTx0 Model}
    \label{fig:eutxo}
\end{figure}
...
We could write a Simeon to Zerepoch compiler that would convert
each Simeon contract into a specific Zerepoch script.
Instead, we chose to implement an interpreter of Simeon contracts as a single Zerepoch script.
This approach has a number of advantages:
\begin{itemize}
\item It is simple: having a single Zerepoch script that implements all Simeon contracts
makes it easier to implement, review, and test what we have done.
\item Implementation is close to the semantics of Simeon, as sketched above and in more detail in~\cite{isola-simeon},
which makes it easier to validate.
\item The same implementation can be used for both on- and off-chain (wallet) execution of Simeon code.
...
e.g.\ in the Simeon Playground.
\item Having a single interpreter for all (or a particular group of) Simeon contracts allows us
  to monitor the blockchain for these contracts, if required.
\item Finally, Bcc nodes could potentially use an optimised interpreter (e.g:~native) just for Simeon contracts, which would save processing time.
\end{itemize}
...
\emph{Redeemer}.
Each step in contract execution is a transaction that spends a Simeon contract transaction output
by providing a valid input as \emph{Redeemer}, and produces a transaction output
with a the remaining Simeon contract and the updated state.
...
As described above, the Simeon interpreter is realised as a \emph{Validation script}.
We can divide the execution of a Simeon Contract into two phases:
...
with the particular Simeon contract in the data script, and protected by the Simeon validator script.
...
\begin{verbatim}
data State = State { accounts    :: Map AccountId Bcc
...
as shown in figure~\ref{fig:simeon-init}.

\begin{figure}[!h]
\centering
\includegraphics[width=1\textwidth]{figures/Simeon3-Figures-002.jpeg}
\caption{Simeon contract initialization}
\label{fig:simeon-init}
\end{figure}

\paragraph{Execution}

Simeon contract execution consists of a chain of transactions,
...
Each execution step is a transaction that spends a Simeon contract transaction output by providing
an expected input in a redeemer script,
and produces a transaction output with a Simeon contract as continuation.

The Simeon interpreter first validates the current contract state: i.e.\
...
\begin{verbatim}
expectedTxOutputs = computeTransaction txInput simeonState simeonContract
\end{verbatim}
...
\noindent
Here we expect Alice to deposit 850 Bcc (850,000,000 Entropic) into her \texttt{aliceAccount} before slot 100.
Otherwise, we \emph{Close} the contract.

If Alice deposits the money before slot 100, money immediately goes to Bob,
by requiring a transaction output of 850 Bcc to Bob's public key address.
Alice must produce the following \emph{Redeemer} to satisfy the Simeon validator:
...
\noindent
Bob is then expected to deposit 1000 Bcc into Alice's account before slot 200.
If he does, the contract is closed, and all remaining balances must be paid out
to their respective owners. In our case, 1000 Bcc must be paid to Alice.

\begin{figure}[!h]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/Simeon3-Figures-004.jpeg}
...
Let us say that Alice has 1000 Entropic on her account, and Bob wants to receive the payment.
...
Except for the transaction that closes a Simeon contract, the Simeon validator script checks that a spending transaction contains a valid continuation
...
Bcc extends its ledger rules to support \emph{forging} of custom currencies and tokens,
effectively embedding support of Ethereum ERC-20 and ERC-721 contracts.
We are working on adding this \emph{multicurrency} support to Simeon.

Simple token creation gives interesting possibilities of representing Simeon contract parties by tokens.
...
In the next two sections, we give an overview of some of the work we have carried out with the aim of helping users be more confident that the Simeon contracts they write do what they expect them to do.
We look at two complementary approaches: static analysis of concrete contracts in this section,  and formal verification of general properties about Simeon in the next.
...
For Turing-complete languages, this process is usually undecidable in the general case, and for Simeon contracts it is often NP-complete in general. However, in practice, SMT (Satisfaction Modulo Theories) solvers are often able to solve moderately large instances of this problem reasonably efficiently.
...
At the time of writing, there are some types of functions and data-types that SBV cannot translate to \texttt{SMTLib}. In particular, there are two that affected the way in which we use SBV with Simeon and that prevent us from using the same version of the semantics for symbolic execution and normal execution.
...
In the following section, we explain how we worked around these limitations and give an overview of how we use SBV to avoid runtime problems with Simeon contracts.

\subsection{Using SBV to analyse Simeon Contracts}

Simeon semantics are designed to use types to prevent many non-sensical contracts from being written. But there are potential problems which are harder to detect until runtime, for example, whether there will be enough money to issue all the payments declared in the contract. At this point, it may already be too late to fix them, and particularly so in the case of blockchain. The Simeon semantics represents these errors that can be found at runtime as \texttt{Warnings}.
...
Fortunately, SBV does support tuples and the \texttt{Either} type; combinations of those two give us essentially the same functionality as custom datatypes that are not recursive and have no type-parameters. We can represent all types that Simeon requires as combinations of \texttt{Either} and tuples, with the exception of the \texttt{Contract} type, but we do not need a symbolic version of the \texttt{Contract} type.
...
\section{Formal verification of the Simeon semantics\label{sec:verification}}

We can also use proof assistants to demonstrate that the Simeon semantics presents certain desirable properties, such as that money is preserved and anything unspent is returned to users by the end of the execution of any contract. In the same way, we can also use this technique to prove properties for particular contracts, or for particular contract snippets, templates, or classes of contracts.

Currently, we have translated the Haskell Simeon semantics to Isabelle as closely as possible. The main differences between the Isabelle and the Haskell semantics are that:
\begin{itemize}
...
One of the dangers of using smart contracts is that a badly written one can potentially lock its funds forever. We want to avoid that feature in Simeon altogether and, with that aim, we proved that Simeon account system does not create or destroy any money. By the end of the contract, all the money paid to the contract must be distributed back, in some way, to a subset of the participants of the contract.
...
For every Simeon \texttt{Contract} there is a slot number after which an empty transaction can be issued that will close the contract and refund all the money in its accounts.
...
Another property of Simeon is that any given finite \texttt{Contract} has an implicit finite bound on the maximum number of \texttt{Transaction}s that it accepts. This is a convenient property because:
...
\section{The Simeon Playground}
\label{sec:playground}

For Simeon to be usable in practice, users need to be able to design and develop Simeon contracts, and also to understand how contracts will behave once deployed to the blockchain, but without before actually deploying them.

The Simeon Playground, a web-based tool that supports the interactive construction, revision, and simulation of smart contracts written in Simeon, provides these facilities, as well as access to a static analysis of contracts (as described in the previous section), an online tutorial for Simeon and a set of example contracts. The playground is available at \url{https://prod.meadow.simeon.tbcodev.io/}.\footnote{Development of the playground is rapid, and the latest, unstable, version is also available at \url{https://alpha.simeon.tbcodev.io/}.}

At the top level, the playground offers three panes: the main \emph{Simulation} pane, as well as panes for developing Simeon contracts, embedded in \emph{Haskell} or using the \emph{Blockly} visual language.

\paragraph{Development}

On the simulation pane, ``pure'' Simeon contacts can be developed: that is, contracts not embedded in a host language. The reason for doing this is two-fold:
\begin{itemize}
    \item There is a shallower learning curve for users who are new to Haskell or programming languages in general. The Simeon constructs are quite simple, and there is no need, at least initially, to learn about Haskell syntax or even variables, functions etc.
...
\end{itemize}
As contracts become larger it makes sense to use another editor in the \emph{Haskell} pane. Here contracts can be written using facilities from \emph{Haskell} to abbreviate and make more readable the description of the contracts. These contracts can then be transferred as a pure Simeon data structure into the simulation pane.

Contracts can also be written using Google's \emph{Blockly} visual programming language, as was earlier described in Meadow~\cite{isola-simeon}. Blockly gives an easy way to introduce the concepts of Simeon to users who have no programming knowledge, and in particular the editor gives users a set of options for each construct as the contract is built. Once a contract has been constructed in Blockly it is possible to transfer that contract to the simulation pane.%SJT Changed from "Simeon editor".
It is also possible to transfer a Simeon contract to Blockly for further editing.
...
The Simeon editor in the unstable version of the playground has an additional feature to aid writing contracts called holes. If we enter the contract \lstinline{?mycontract}
%SJT: enter it where?
we will be presented with a dropdown list of values that could be used.

In our case \lstinline{?mycontract} must be a \lstinline{Contract} of some sort, and so we are offered a choice of \lstinline{Contract} constructors from a dropdown list. If we choose \lstinline{Pay} then the Simeon editor will automatically fill in a skeleton \lstinline{Pay} contract with new holes where we need to provide values.
\begin{verbatim}
...
Users can at any point save the current contract directly to a Github Gist, as well as being able to re-load contracts from Github Gists. There are also some demo contracts that can be loaded in their Haskell and Simeon versions.

\paragraph{Simulation}

Contracts written in the Simeon editor are parsed in real-time and if there are no errors (and no holes) then the contract is analysed to discover which actions a user could take to progress the contract. These actions are displayed in the ``Input Composer'' above the editor. Consider the following example contract:
\begin{verbatim}
    When [Case (Deposit (AccountId 0 "investor")
         ...
         Close
\end{verbatim}
In this case, the only action a user can take to progress the contract is to accept a deposit of 1000 BCC
%SJT: we need to sort out the Bcc/Lovelance mismatch.
...
\noindent
A user can then apply this transaction and in the example above this would result in the state pane showing a single payment and in addition the contract in the Simeon editor will have been reduced to \lstinline{Close}.
...
An earlier paper~\cite{cryptoeprint:2016:1156} reviews smart contracts on blockchain. Here we look at a number of recent systems that bear direct comparison with Simeon.

The Ethereum system~\cite{wood2014ethereum} provides an exemplar Turing-complete language for  the EVM virtual machine, and a higher-level programming language, Solidity, that compiles into EVM code. Both these languages are general-purpose, and so have more akin with Zerepoch~\cite{ZerepochPlatform} rather than Simeon.
...
account by our notion of commitments. They take no account of commitments or timeouts as our approach does, and so are unable to guarantee some of the properties -- such as a finite lifetime -- built into Simeon by design.

BitML~\cite{BitML} is a DSL for specifying Simeon-like contracts that regulate transfers on the Bitcoin blockchain. BitML is implemented via a compiler that translates contracts into standard Bitcoin transactions. Participants execute a contract by appending these transactions on the Bitcoin blockchain, according to their strategies, which also involve the exchange of bitstrings that guarantee to a very high probability the correctness of contract execution. Simeon is directly implemented by an interpreter which could also be implemented on a covenant-based~\cite{covenants} extension of the Bitcoin blockchain.

\section{Conclusions and future work}

Rather than aiming to be general-purpose, Simeon is a DSL designed to support financial contracts on blockchain. We leverage its specificity in our work on static analysis and verification, where we are able to deliver much greater impact and focus than we could for a general-purpose language. We are able to shape the development and simulation environment to give stronger user support too. Moreover, Simeon  presents a model for how other DSLs can be built in this space, supporting different domains such as provenance in the supply chain. 

Defining the language by means of an executable reference semantics means that we can, as well as directly executing this semantics, generate an on-chain interpreter for it and simulate it in browser using the Haskell-like languages Zerepoch and PureScript. This is made particularly straightforward by working in the subset of Haskell that directly corresponds to what translates directly into these languages.

Our medium term aim is launching on Bcc blockchain itself, by which time we expect to have added multiple currencies to Simeon, as well as making (roles in) Simeon contracts tradeable, through tokenising contract roles.
