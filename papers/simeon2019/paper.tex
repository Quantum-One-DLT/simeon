...
Because Simeon is a DSL, we are able to build special purpose tools and techniques to support it. Crucially in a financial environment, we are able to \emph{exhaustively analyse contracts} without executing them, so that we can, for instance, check whether any particular contract is able to make all the payments it should: in the case it is not, we get an explicit example of how it can fail. This analysis (Section \ref{sec:static}) is built into the Simeon Playground. Finally (Section \ref{sec:verification}) we are able to use formal verification to prove properties of the implementation of Simeon, including a guarantee that ``money in $=$ money out'' for all contracts.

Building on~\cite{isola-simeon}, the specific contributions of this paper are threefold. We describe revised versions of Simeon and its Playground, we describe the implementation of Simeon on the Bcc blockchain, and we present both automated static analysis and machine-supported verification for Simeon.

\section{Simeon revised: version 3.0}
\label{sec:overview}
...
%Contracts in Simeon run on a blockchain, but need to interact with the off-chain world. 
...
Simeon is specified by an executable semantics written in Haskell, but to make it usable in practice with financial contracts, it needs to be implemented on a blockchain. In this section, we explain how Simeon is executed on the Bcc blockchain using an interpreter \footnote{The implementation is available at \url{https://github.com/The-Blockchain-Company/zerepoch/blob/0ca9af4f6614d591de7ebbe4dd759ce122d74efd/simeon/src/Language/Simeon/Semantics.hs}.} written in the Zerepoch programming language.
...
Bcc aims to support smart contracts during its Sophie release in 2020.
...
It extends the Bitcoin model by allowing transaction outputs to hold a \emph{Datum}.
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=4in]{figures/Simeon3-Figure-1-crop.pdf}
...
one must create a transaction (a \emph{Context}) that has an \emph{input} that references the transaction output,
and contains a \emph{Redeemer}, such that \texttt{Validator(Datum, Redeemer, Context)} evaluates to $True$.
...
Instead, we chose to implement a Simeon interpreter as a single Zerepoch script.
...
where, at each stage, the remaining contract and its state are passed through the \emph{Datum},
and actions/inputs (i.e. \emph{choices} and \emph{money deposits}) are passed via the 
%SJT: is this right?
%AN: yes. [Input] is precisely the redeemer script. We could say that Value and SlotInterval are parts of Context,
...
We store the remaining contract in the \emph{Datum}, which makes it visible to everyone.
...
with the particular Simeon contract in the \emph{Datum}, and protected by the Simeon validator script.
...
where the remaining contract and state are passed through the \emph{Datum},
...
\noindent
Here we expect Alice to deposit 850 Bcc (850,000,000 Entropic) into her account \texttt{aliceAccount} before slot 100.
...
to their respective owners; in this case, 1000 Bcc must be paid to Alice. If Bob does not pay, then Alice has lost her money, because this is an unsecured loan.

\begin{figure}[!h]
    \centering
    \includegraphics[width=4in]{figures/Simeon3-Figure-2-crop.pdf}
...
Bcc extends its ledger rules to support \emph{forging} of custom currencies and tokens.%effectively embedding support of Ethereum ERC-20 and ERC-721 contracts. 
...
Simeon semantics represents errors that can be found at runtime as \texttt{Warnings}.
...
SBV does not currently seem to support in general the use of custom datatypes. Fortunately, SBV supports tuples and the \texttt{Either} type. We can represent all types that Simeon requires as combinations of \texttt{Either} and tuples, with the exception of the \texttt{Contract} type, but we do not need a symbolic version of the \texttt{Contract} type because we know its value at the time of analysis.
...
We can also use proof assistants to demonstrate that the Simeon semantics presents certain desirable properties, such as that money is preserved and anything unspent is returned to users by the end of the execution of any contract. Formal statements and proofs of these properties are given in~\cite{simeon-github}.


Currently, we have translated the Haskell Simeon semantics to Isabelle while keeping both versions as close as possible, but we decided to make them different in two main aspects:
\begin{itemize}
...
For Simeon to be usable in practice, users need to be able to design and develop Simeon contracts, and also to understand how contracts will behave once deployed to the blockchain, but without actually deploying them.
...
On the simulation pane, ``pure'' Simeon contacts can be developed directly, not embedded in another language. Two reasons for doing this are:
\begin{itemize}
...
%    \vspace*{-0.25in}
    \includegraphics[scale=0.18]{figures/hole_options_2.png}
\end{wrapfigure}
...
\end{verbatim}
In this case, the only action a user can take to progress the contract is to accept a deposit of 1000 BCC from the guarantor to the investor's account. Because of this, the playground can display this action in the input composer.

\begin{wrapfigure}[4]{l}{0.47\textwidth}
    \vspace*{-0.28in}
    \includegraphics[scale=0.15]{figures/input_composer.png}
\end{wrapfigure}
...
\begin{wrapfigure}[5]{l}{0.47\textwidth}
    \vspace*{-0.32in}
    \includegraphics[scale=0.15]{figures/tx_composer.png}
\end{wrapfigure}
...
In the remainder of this section we look at a number of recent systems that bear direct comparison with Simeon; an earlier paper~\cite{cryptoeprint:2016:1156} gives a detailed review of smart contracts on blockchain. 
...
account by our notion of commitments. They take no account of commitments or timeouts as our approach does, and so are unable to guarantee some properties -- such as a finite lifetime -- built into Simeon by design.
...
Defining the language by means of an executable reference semantics means that we can, as well as directly executing this semantics, generate an on-chain interpreter for it and simulate it in browser using the Haskell-like languages Zerepoch and PureScript. This is particularly straightforward when working with a subset of Haskell that is represented in the same way on these languages.
