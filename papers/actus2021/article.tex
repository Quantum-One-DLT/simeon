...
\begin{document}
\title{Standardized crypto-loans on\\ the Bcc blockchain}%\thanks{Supported by Bcc Foundation.}}
...
\email{dmytro.kondratiuk@tbco.io, pablo.lamela@tbco.io, alexander.nemish@tbco.io, simon.thompson@tbco.io}  \and
...
using the Simeon domain-specific language and the ACTUS standard for financial contracts.

\keywords{ACTUS \and blockchain \and Bcc \and finance \and Haskell \and Simeon
...
Smart contracts -- programs that run in a blockchain environment --  can be defined in a variety of ways~\cite{cryptoeprint:2016:1156}. Many such approaches are general purpose, and can be used to program any kind of contract it makes sense to run on a blockchain; moreover, they tend to be expressive enough to be Turing complete (in some cases with restrictions on the runtime environment). For example, Zerepoch, the general-purpose language running on the Bcc blockchain~\cite{utxo-vs-accounts}, is a dialect of Haskell. Another approach is to develop special-purpose or \emph{domain-specific languages} (DSLs) which embody a particular application domain: Simeon~\cite{simeon} is a high-level DSL for writing
financial contracts on the Bcc blockchain. 

In this paper we explore ways in which contracts described in ACTUS (Algorithmic Contract Types Unified Standards) can be defined in the contract languages Simeon, Zerepoch and Haskell. Of course, Zerepoch or Haskell are able to express these contracts, but rendering them in Simeon brings extra advantages. Simeon is defined to provide a range of guarantees by design: a Simeon contract will only make a finite number of interactions with its environment, and its lifetime can be read off from the code for a contract; moreover, when the contract terminates, any assets held by the contract will automatically be returned to the participants. None of these guarantees can be provided by a general purpose language. 

Each Simeon contract has a finite set of possible execution paths, and so it is possible to analyse the complete behaviour of a contract without running it. Such \emph{static analysis}, based on SMT solving~\cite{smt}, can be used to check properties of a contract; for example, it is possible to check whether a contract will honour all the \texttt{Pay} constructs that it contains, however it is executed. In the case that a \texttt{Pay} can fail, the analysis gives an example trace showing how that failure happens.
The language design and static analysis provide assurance that Simeon contracts are much less likely to ``misbehave'' than contracts written in a general-purpose language like Solidity. 

In implementing ACTUS on Bcc we are able to provide further assurance in three other ways. First, we are able to use the declarative nature of Haskell to transliterate ACTUS formulas term-by-term into an executable form in Haskell. Secondly, we are able to use random, property-based testing to validate the Haskell implementation against another written in Java. Finally, we are able to automatically \emph{generate} ACTUS contracts in Simeon from the terms -- i.e. parameters -- of the contracts; for a simple loan these would include the start and end dates of the loan and the amount loaned (the `principal'). The generated contracts use the executable spec in calculating values of cash flows in the Simeon contracts.

The contribution of our work is to show that that it is possible to implement financial contracts on blockchain in a way that \emph{multiple forms of assurance} are provided: from the language itself, from the static analysis, and from custom property verification. The development environment for Simeon, the Simeon Playground~\footnote{\url{https://alpha.simeon.tbcodev.io/\#/}}, also provides a simulation environment for contracts for stepping forwards (and backwards) through contract execution, and thus allowing users to validate that contracts perform as they should. In addition, implementing ACTUS provides a suitable benchmark against which to assess the design of Simeon; we illustrate how the implementation has led to the addition of a conditional expression construct to the language. 

In the remainder of the paper, Section \ref{background} covers the relevant financial background, including the ACTUS financial standard. Section \ref{executable} builds an executable specification of ACTUS in Haskell, and this is used in Section \ref{generation} to generate  the Simeon code for an ACTUS contract from the contract terms. Section \ref{tokenization} explains how tokens are used to represent ownership of roles in a running contract, and Section \ref{assurance} describes how we provide assurance that  contracts behave as they should. Section \ref{related} examines related work and Section \ref{conclusion} concludes. 

A note on notation: \texttt{typewriter} font will be used for Simeon constructs
...
%This section introduces the Simeon domain-specific language (DSL) and the ACTUS standard for financial contracts.
%
%\subsection{Simeon}
%
%Simeon~\cite{simeon} is a high-level DSL for writing
%financial contracts on blockchain. Simeon is defined
%by an executable semantics in Haskell, and has been implemented on
%the UTxO-based Bcc blockchain. 
%% SJT added this
%%dk14 OK
%Simeon contracts are finite, and moreover have defined finite lifetime. This means that it is possible to perform \emph{static analysis} on them. For example, it is possible symbolically to check \emph{all} execution paths without running a contract, and in particular checking that every \texttt{Pay} construct can be executed in full.
%
%The main advantage of using Simeon to carry ACTUS logic is enhanced
...
\end{equation*}
\noindent
In the case of the Bcc blockchain, these values are usually
...
From a Simeon DSL perspective,
the exact mechanism that provides external data is less important,
as Simeon abstracts over IO by requiring a particular type of input -- a 
...
\item a formula representing a piece of \emph{abstract syntax}, e.g. a Simeon \texttt{Value} or \texttt{Observation}, that could
...
across ACTUS related services maintained by Bcc we rely on ``superposed''
...
\section{Generating Simeon contracts from standardised ACTUS contract terms}
\label{generation}

In this section we describe how concrete Simeon contracts are generated from the terms -- i.e. parameters -- of standard ACTUS contracts. We also describe the implementation of the system, and reflect on the limitations of generating contracts in Simeon, where contracts have a predefined lifetime and a predefined collection of interactions with contract participants.
...
\noindent
where the component \emph{receiveData} asks an oracle for Simeon Choice if needed, and
...
The principal components of the system are shown in Figure \ref{fig1}. There are three categories of components representing ACTUS functions: specification with formulas, formula wiring for fixed rates (produces precomputed payoffs) and formula wiring for variable rates, which  produces Simeon-code that computes payoffs. Different types of wiring correspond to different implementations of \emph{ActusOps} implementations (either \emph{Double} or \emph{Value}). 
...
Simeon contracts are finite, and in particular Simeon itself does not have constructs for functions or recursion; these \emph{are} available in the Haskell and JavaScript embeddings of Simeon, but they are unrolled on translation to pure Simeon. Naive usage of the \texttt{If} operator in Simeon could lead to exponential growth of a contract, as in this pesudocode example:
...
\end{alltt}
\noindent
Translating this to Simeon would inline contents of \texttt{continue()} twice and, given that ACTUS
...
Simeon doesn't allow contracts that run indefinitely, even if their recursion
...
For numeric types Simeon supports Integers, while ACTUS is expressed in terms
...
\begin{verbatim}
  ...
  a * b = Scale (1 % simeonFixedPoint) $ MulValue a b 
                            -- x/n * y/n = (x * y)/n^2
\end{verbatim}
We scale all numbers with \emph{simeonFixedPoint} factor - which only requires modification of \emph{MulValue}.
% SJT this needs more explanation here ^^^
% dk14 I've added some
\noindent
We plan to move Simeon to fixed-precision numbers for the on-blockchain implementation available later in 2021.

\subsection{Representing Actus state in a Simeon contract}

The Simeon DSL does not support any notion of records,
the variables can only be of type ``Integer''. In order to
map contract state -- \emph{ContractStatePoly} -- we pack a set of Simeon
...
finally unpack \emph{ContractStatePoly} into a set of Simeon variables representing the 
 state at $t$:
...
\paragraph{Representing state transitions in Simeon}

Simeon is a declarative language, and so in particular it does not support mutable variables. We therefore represent the state at stage $t$ ($st_{t}$) literally through this naming convention:
...
\caption{Actus Labs - an online tool for generating Actus contracts for Simeon.} 
\label{fig3} 
\end{figure} 

In order to demonstrate and test the capabilities of Actus generators,
a visual online Blockly-based tool was developed for the Simeon Playground.
The Actus Labs tool, shown in Figure \ref{fig3}, allows users to construct contract terms visually
to generate a corresponding Simeon contract and then to try it out in a simulation 
environment.

\section{Tokenization}
\label{tokenization}

Every participant of a Simeon contact %ACTUS contract 
...
This section explains how we provide assurance to users of our Simeon ACTUS contracts by means of property-based testing and SMT-based static analysis.
...
other properties of a Simeon contract. This could be enhanced
with Simeon's static analysis feature by utilising the \texttt{Assert} operator:
...
let contractTerms = sample(qcgenerator) 
    contract = generateSimeonContract(contractTerms) 
...
Using static analysis for Simeon~\cite{simeon-efficient} it is possible fully to check a particular contract instead of using random
...
By design, the Simeon interpreter always refunds any assets held in a
...
Unlike current mainstream DeFi lending approaches, our Simeon ACTUS implementation relies on trade-matching instead of pooling.
...
like Simeon rather than general-purpose one like Java or Haskell.

\section{Conclusion}
\label{conclusion}

The Simeon language was explicitly designed as a set of building blocks
for financial contracts that could be combined by anyone familiar
with basic programming. The Simeon ACTUS generators improve on that by
providing a way to automatically combine blocks based on standardised
requirements specified by the user. Simeon ACTUS also provides a toolkit for cross-testing
